...
for i=1:num
    ...
    tempDGE=DeltaG_with_various_rev(SSP(:,i),GE,GI,rev);
    DGE(i)=-tempDGE(posi);
...
%figure plot
subplot(2,3,count);
d1=plot(GEGI,DGE,'ko','Markersize',5);
hold on;
modelfun=@(k,x)k*x;
...
% title(['\epsilon=',num2str(rev*15-70),'   alpha=',num2str(alpha)]);
...
set(gcf,'Units','centimeters','Position',[6 6 14 8]);
